...
Generated Kokoro audio is the preferred playback path on iOS and other browsers. Browser Web Speech is only a fallback when no generated audio is playable. iOS Web Speech exposes Apple system voices, not repository-generated Kokoro voices, so it cannot reproduce the generated voice through `speechSynthesis`.
